...
defenders, goalkeeper, shooter), the Hybrid CNN+Transformer at
$128{\times}128$ outperforms the same-resolution pure CNN on every
headline metric across three training seeds: ROC-AUC $+0.004$,
PR-AUC $+0.035$, F1 $+0.009$, Platt Brier $-0.001$. The lift over
the lower-resolution CNN-64 baseline is smaller but consistent
(ROC-AUC $+0.003$, PR-AUC $+0.014$). The lift grows
...
location, body part, assist type, play pattern) dominate practice;
Anzer and Bauer report AUC values up to approximately $0.83$ using
synchronized positional and event data~\cite{anzer2021xg}. Such
models encode spatial structure through engineered features rather
than exposing the raw freeze-frame geometry directly to the model.
Image-based xG addresses this gap by rendering the StatsBomb shot
freeze-frame as a top-down image and letting a vision model learn
the spatial layout directly. Earlier CNN work used top-down position
images for goal-scoring opportunity prediction~\cite{wagenaar2017xg};
xG-CNN later applied CNNs directly to image-based xG from StatsBomb
freeze frames~\cite{matteotti2024xgcnn}.
...
\textbf{At matched resolution, Hybrid 128 outperforms CNN 128 on every
headline metric; the same-resolution PR-AUC gain ($+0.035$) is the
largest effect.}}
\label{tab:headline}
\resizebox{\textwidth}{!}{%
...
Table~\ref{tab:headline} reports the controlled six-cell ablation. At
matched resolution, Hybrid 128 outperforms CNN 128 on every headline
metric: ROC-AUC $+0.004$, PR-AUC $+0.035$, F1 (val-best) $+0.009$,
Platt Brier $-0.001$. The same-resolution PR-AUC gap is the largest
effect in the table and exceeds the observed seed variability by
several multiples. As a more conservative cross-resolution check,
Hybrid 128 also beats the CNN-64 baseline on every metric (ROC-AUC
$+0.003$, PR-AUC $+0.014$, F1 $+0.006$, Platt Brier $-0.001$), with
PR-AUC and Brier still exceeding seed variability.
...
64 tokens ($+0.035$). We interpret this as evidence that the
...
hybrid at $128{\times}128$ outperforms the same-resolution CNN on
every headline metric and also exceeds the lower-resolution CNN-64
baseline. The lift is
